...
import c5t4.service.JuegoService;
...
public class JuegoResource {
    ...
    @POST
    public Response crear(Juego juego) {
        try {
    ...
    @PUT
    @Path("/{id}")
    public Response actualizar(@PathParam("id") Long id, Juego juego) {
        try {
            Juego juegoActualizado = juegoService.actualizar(id, juego);
            return Response.ok(juegoActualizado).build();
        } catch (IllegalArgumentException e) {
            return Response.status(Response.Status.NOT_FOUND)
                    .entity(new ErrorResponse(e.getMessage()))
                    .build();
